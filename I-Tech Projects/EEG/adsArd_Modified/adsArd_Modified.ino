//#define BAUD_RATE  9600//specifies BlueTooth speed. top speeds appear to be around 115200 for boards with 16MHZ
...
void checkSerialNum() { //handle commands to ADS device

  unsigned char  val = Serialread();      
  using namespace ADS1298; 
  ...
  if ((val >= 0x20) && (val <= 0x3F)) { //RREG (Read register) command  
      for (int i = 0; i < 10; ++i) if (Serialavailable() < 1) delay(1); 
      if(Serialavailable() < 1) return;
      unsigned char  numSerialBytes = Serialread()+1;
  ...
  if ((val >= 0x40) && (val <= 0x5F)) { //WREG (Write Register) command
      for (int i = 0; i < 5; ++i) if (Serialavailable() < 1) delay(1); 
      if(Serialavailable() < 1) return;
      unsigned char  numSerialBytes = Serialread()+1;
      for (int i = 0; i < 5; ++i) if (Serialavailable() < numSerialBytes) delay(1); 
      if(Serialavailable() < numSerialBytes) return;
      unsigned char serialBytes[numSerialBytes];
      for (int i = 0; i < numSerialBytes; ++i) serialBytes[i] = Serialread(); 
      digitalWrite(IPIN_CS, LOW);
      SPI.transfer(val);
...
void setup(){
  ...
  WiredSerial.print("IDval: "); WiredSerial.println(out);
  ...
  WiredSerial.print("gMaxChan: "); WiredSerial.println(gMaxChan);
  //start serial port
//  Serial1.begin(BAUD_RATE);
//  Serial1.flush(); //flush all previous received and transmitted data
   //use native port on Due
  ...
  adsSetup();
  WiredSerial.println("Ready");//optional - sets up device - the PC can do this as well       
} //setup()
