...
#define min_30 1800000
#define min_35 2100000
#define min_40 2400000
#define min_45 2700000
#define min_50 3000000
#define min_60 3600000
...
byte colPins[COLS] = {35, 36 , 37, 40}; //connect to the column pinouts of the keypad
...
char  pumpFlag,matchFlag=0;
unsigned long  startTime,pumpOFF;
int valve[13]={29,28,27,26,25,24,23,22,4,5,6,7};
int c=0;
bool pump=LOW,waitFlag=LOW,set=HIGH; 
int orderValve[12];
int orderTime[12];
float EC_ppm;
...
void getEC(){
      long voltageEC=0.0;
      float voltageECavg=0.0;
      for(int i=0; i<50; i++){
        voltageEC+=analogRead(EC_PIN);
        delay(1);
      }
      voltageECavg =voltageEC/50.0;
      voltageECavg =(voltageECavg*5.0)/1024.0;
      Serial.print("voltage: "); Serial.println(voltageECavg);    
      long ecValue=1000000*voltageECavg/RES2/ECREF;
...
void Preset()
{
  ...
  if (strcmp(curDisplay, "C16D") == 0) {
    ...
    lcd.print("CONFIGURATION#6");
    lcd.setCursor(2, 1);
    lcd.print("Multiple Selected");
    lcd.setCursor(2, 2);
    lcd.print("Enter to Reselect");
    lcd.setCursor(2, 3);
    lcd.print("Cancel to Abort");
    } 
    else{  
...
void process()
{
  ...
  {
    if(config6Flag){
      digitalWrite(orderValve[c],HIGH);
      Delay(orderTime[c]);
    }
    else{
    if(config5Flag && set){
...
void sortOrder(){
  for(int i=0; i<12; i++){
    orderValve[selOrder[i]]=valve[i];
    orderTime[selOrder[i]]=selTime[i];
  }
}
